...
fn find_cut(
    ...
    for line in lines {
        if frame_u.contains(&line.rect)
            && line.rect.height() <= max_line_height
            && y_pos_u >= line.rect.min.y
            && y_pos_u < line.rect.max.y
        {
...
impl Reader {
    ...
    fn load_text(&mut self, location: usize) {
        ...
        let words = doc.words(loc).map(|(words, _)| words).unwrap_or_default();
            ...
            if let Some(toc) = self.toc().or_else(|| doc.toc()) {
                let chap_offset = if dir == CycleDir::Previous {
                    doc.chapter(current_page, &toc)
                        ...
                        .and_then(|chap_offset| {
                            if chap_offset < current_page {
                                Some(chap_offset)
                            } else {
                                None
                            }
    ...
    fn go_to_bookmark(
        ...
        let loc_bkm = self.info.reader.as_ref().and_then(|r| match dir {
            ...
            CycleDir::Previous => r.bookmarks.range(..self.current_page).next_back().cloned(),
        });
            ...
            ScrollMode::Screen => {
                ...
                if next_top_offset < 0 {
                    ...
                    if let Some(previous_location) =
                        doc.resolve_location(Location::Previous(location))
                    {
...

                {
                    ...
                    if let Some((lines, _)) = doc.lines(Location::Exact(location)) {
                        if let Some(mut y_pos) = find_cut(
                            &frame,
                            frame.min.y + next_top_offset,
                            scale,
                            LinearDir::Forward,
                            &lines,
                        ) {
    ...
    fn go_to_neighbor(
        ...
        let loc = {
            let neighloc = match dir {
                CycleDir::Previous => match self.view_port.zoom_mode {
                    ...
                    ZoomMode::FitToWidth => match self.view_port.scroll_mode {
                        ScrollMode::Screen => {
                            ...
                            let available_height =
                                self.rect.height() as i32 - 2 * self.view_port.margin_width;
                            let mut height = 0;
                        ...
                        ScrollMode::Page => {
                            ...
                            if self.view_port.page_offset.y > 0 {
                                self.view_port.page_offset.y =
                                    (self.view_port.page_offset.y - available_height).max(0);
                ...
                CycleDir::Next => match self.view_port.zoom_mode {
                    ...
                    ZoomMode::FitToWidth => match self.view_port.scroll_mode {
                        ...
                        ScrollMode::Page => {
                            ...
                            if frame_height < available_height || next_top_offset == frame_height {
                                ...
                                self.view_port.page_offset.y =
                                    next_top_offset.min(frame_height - available_height);
                                Location::Exact(current_page)
                            }
                        }
                    },
    ...
    fn update_noninverted_regions(&mut self, inverted: bool) {
        ...
        if inverted {
            for chunk in &self.chunks {
                if let Some((images, _)) = self
                    .doc
                    .lock()
                    .unwrap()
                    .images(Location::Exact(chunk.location))
                {
                ...
                ScrollMode::Screen => {
                    ...
                    while height < available_height {
                        ...
                        if let Ok(mut doc) = self.doc.lock() {
                            if let Some(next_location) =
                                doc.resolve_location(Location::Next(location))
                            {
                                location = next_location;
                            } else {
                                break;
                    ...
                    if height > available_height {
                        if let Some(last_chunk) = self.chunks.last_mut() {
                            ...
                            let mut doc = self.doc.lock().unwrap();
                            if let Some((lines, _)) =
                                doc.lines(Location::Exact(last_chunk.location))
                            {
                                let pixmap_frame = self.cache[&last_chunk.location].frame;
                                if let Some(mut y_pos) = find_cut(
                                    &pixmap_frame,
                                    last_chunk.frame.max.y,
                                    last_chunk.scale,
                                    LinearDir::Backward,
                                    &lines,
                                ) {
                        ...
                        let actual_height: i32 =
                            self.chunks.iter().map(|c| c.frame.height() as i32).sum();
    ...
    fn toggle_results_bar(&mut self, enable: bool, rq: &mut RenderQueue, _context: &mut Context) {
        if let Some(index) = locate::<ResultsBar>(self) {
            ...
            if let Some(ref s) = self.search {
                let results_bar = ResultsBar::new(
                    rect,
                    s.current_page,
                    s.highlights.len(),
                    s.results_count,
                ...
                self.children
                    .insert(index, Box::new(results_bar) as Box<dyn View>);
                let separator = Filler::new(sp_rect, BLACK);
                self.children
                    .insert(index, Box::new(separator) as Box<dyn View>);
                rect.absorb(&sp_rect);
    ...
    fn toggle_search_bar(
        ...
        if let Some(index) = locate::<SearchBar>(self) {
            ...
            self.children
                .insert(index, Box::new(search_bar) as Box<dyn View>);
            ...
            if index == 0 {
                rq.add(RenderData::new(
                    self.child(index + 2).id(),
                    *self.child(index + 2).rect(),
                    UpdateMode::Gui,
                ));
            }
    ...
    fn toggle_bars(
        ...
        if let Some(top_index) = locate::<TopBar>(self) {
            ...
            self.children
                .insert(index, Box::new(top_bar) as Box<dyn View>);
            index += 1;
            ...
            self.children
                .insert(index, Box::new(bottom_bar) as Box<dyn View>);
    ...
    fn toggle_margin_cropper(
        ...
        if let Some(index) = locate::<MarginCropper>(self) {
            ...
            self.children
                .push(Box::new(margin_cropper) as Box<dyn View>);
        }
    }
    ...
    fn toggle_edit_note(
        ...
        if let Some(index) = locate_by_id(self, ViewId::EditNote) {
            ...
            if self
                .focus
                .map(|focus_id| focus_id == ViewId::EditNoteInput)
                .unwrap_or(false)
            {
                self.toggle_keyboard(false, None, hub, rq, context);
            }
    ...
    fn find_page_by_name(&self, name: &str) -> Option<usize> {
        self.info.reader.as_ref().and_then(|r| {
            if let Ok(a) = name.parse::<u32>() {
                ...
                r.page_names
                    .iter()
                    .filter_map(|(i, s)| Roman::from_str(s).ok().map(|b| (*b, i)))
                    .filter(|(b, _)| *b <= *a)
                    .max_by(|x, y| x.0.cmp(&y.0))
                    .map(|(b, i)| *i + (*a - b) as usize)
            } else {
        ...
        for chunk in &self.chunks {
                ...
                .and_then(|s| s.highlights.get(&chunk.location))
            {
                for rects in groups {
                    ...
                    for rect in rects {
                        let rect =
                            (*rect * chunk.scale).to_rect() - chunk.frame.min + chunk.position;
                ...
                if let Some(annotations) = self.annotations.get(&chunk.location) {
                    for annot in annotations {
                        ...
                        if let Some(text) = self.text.get(&chunk.location) {
                            ...
                            for word in text
                                .iter()
                                .filter(|w| w.location >= start && w.location <= end)
                            {
